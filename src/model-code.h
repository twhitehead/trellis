...
World_SVarieties_SSIndividuals World_next(Space const space,
  ...
  if (world.year == 0) {
    // Add initial varieties and individualss
    Variety const varieties[] = {
      // Paper birch
      Variety_raw(Variety_Type_PaperBirch,
                  2.0, 23.0,
                  0.25, 1.5,
                  0.015, 0.9, 90.0,
                  15.0,
                  0.95, 10.0, 500.0),
      // White pine
      Variety_raw(Variety_Type_WhitePine,
                  2.0, 40.0,
                  0.18, 1.2,
                  0.003, 0.9, 70.0,
                  10.0,
                  0.95, 10.0, 500.0),
      // Sugar maple
      Variety_raw(Variety_Type_SugarMaple,
                  2.0, 25.0,
                  0.1, 0.8,
                  0.008, 0.4, 70.0,
                  5.0,
    ...
    static const UInt varieties_number = sizeof(varieties)/sizeof(varieties[0]);

    for (UInt varieties_index=0; varieties_index < varieties_number; varieties_index+=1) {
      // Add variety
      avarieties = AVarieties_append(avarieties, varieties[varieties_index]);

      // Add individuals
      AIndividuals aindividuals = AIndividuals_begin();

      UInt const individuals_number = space.size_x*space.size_y/(world.cell_diameter*world.cell_diameter);
      for (UInt individuals_iterator=0; individuals_iterator < individuals_number; individuals_iterator+=1) {
        Float32 x, y;
...
Variety Variety_raw(Variety_Type const type,
                    ...
                    Float32 const growth_rate, Float32 const growth_decay,
                    ...
                    Float32 const dispersal_method,
                    Float32 const dispersal_short, Float32 const dispersal_long) {
  Variety const variety = { .type = type,
                            .height_mature = height_mature, .height_maximum = height_maximum,
                            .growth_rate = growth_rate, .growth_decay = growth_decay,
...
FILE* Variety_saveFP(Space const space, World const world, Variety const variety,
                     char const* const name, FILE* file) {
  if ( fprintf(file, "%d %g %g %g %g %g %g %g %g %g %g %g\n",
               variety.type,
               variety.height_mature, variety.height_maximum,
               variety.growth_rate, variety.growth_decay,
...
Variety_FILE_UInt64 Variety_loadFP(Space const space, World const world,
  ...
  records = fscanf(file, "%d %g %g %g %g %g %g %g %g %g %g %g\n",
                   &type,
                   &variety.height_mature, &variety.height_maximum,
                   &variety.growth_rate, &variety.growth_decay,
                   &variety.mortality_intrinsic, &variety.mortality_initial, &variety.mortality_decay,
...
AIndividuals Individual_next(AIndividuals aindividuals, Space const space,
  ...
  if (individual.height >= variety.height_mature) {
    UInt32 const seeds = floor( variety.fecundity_maximum * individual.height/variety.height_maximum );
