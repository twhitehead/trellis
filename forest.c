...
//
SIndividuals sindividuals(UInt64 number, SIndividuals_ sindividuals_);
...
//
IZ iz(Bool valid, UInt64 z);
IZ iz_valid_set(IZ iz, Bool valid);
IZ iz_z_set(IZ iz, UInt64 z);
IZ iz_next_box(IZ iz, UInt64 box_ul_z,UInt64 box_lr_z);
...
IIndividuals iindividuals_valid_set(IIndividuals iindividuals, Bool valid);
IIndividuals iindividuals_index_set(IIndividuals iindividuals, UInt64 index, SIndividuals1 sindividuals1);
...
IIndividuals iindividuals_first_z(SIndividuals sindividuals, IZ iz);
IIndividuals iindividuals_next_z(IIndividuals iindividuals, IZ iz);
IIndividuals iindividuals_first_box(SIndividuals sindividuals, UInt64 box_ul_z,UInt64 box_lr_z);
IIndividuals iindividuals_next_box(IIndividuals iindividuals, UInt64 box_ul_z,UInt64 box_lr_z);
...
SIndividuals_ aindividuals_sort_both(SIndividuals_ sindividuals_,
...
SRIndividualsIn srindividuals_in_begin(Space space, Variety variety, SIndividuals sindividuals);
void srindividuals_in_end(SRIndividualsIn srindividuals_in);

SRIndividualsOut srindividuals_out_begin(Space space, Variety variety, SIndividuals sindividuals);
void srindividuals_out_end(SRIndividualsOut srindividuals_out);

SSRIndividualsIn ssrindividuals_in_begin(Space space, SVarieties svarieties, SSIndividuals ssindividuals);
void ssrindividuals_in_end(SSRIndividualsIn ssrindividuals_in);

SSRIndividualsOut ssrindividuals_out_begin(Space space, SVarieties svarieties, SSIndividuals ssindividuals);
void ssrindividuals_out_end(SSRIndividualsOut ssrindividuals_out);
...
//
void world_save(World world, char* name);
void world_save_fp(FILE* file, World world, char* name);

World world_load(char* name);
World world_load_fp(FILE* file, char* name);
...
//
RSpace reduce_space_first(Space space);
RSpace reduce_space_rest(Space space, Variety variety, Individual individual);
RSpace reduce_space_merge(RSpace rspace0, RSpace rspace1);

RVariety reduce_variety_first(Space space, Variety variety);
RVariety reduce_variety_rest(Space space, Variety variety, Individual individual);
RVariety reduce_variety_merge(RVariety rvariety0, RVariety rvariety1);

Box reduce_individual_in_bound(Space space, Variety variety0, Individual individual0, Variety variety1);
Bool reduce_individual_in_filter(Space space, Variety variety0, Individual individual0,
                                 Variety variety1, Individual individual1);

RIndividualIn reduce_individual_in_first(Space space, Variety variety, Individual individual);
RIndividualIn reduce_individual_in_rest(Space space, Variety variety0, Individual individual0,
                                        Variety variety1, Individual individual1);
RIndividualIn reduce_individual_in_merge(RIndividualIn rindividual_in0, RIndividualIn rindividual_in1);

Box reduce_individual_out_bound(Space space, Variety variety0, Individual individual0, Variety variety1);
Bool reduce_individual_out_filter(Space space, Variety variety0, Individual individual0,
                                  Variety variety1, Individual individual1);

RIndividualOut reduce_individual_out_first(Space space, Variety variety, Individual individual);
RIndividualOut reduce_individual_out_rest(Space space, Variety variety0, Individual individual0,
                                          Variety variety1, Individual individual1);
RIndividualOut reduce_individual_out_merge(RIndividualOut rindividual_in0, RIndividualOut rindividual_in1);

//
Space next_space(Space space, RSpace rspace, Thread thread);
Variety next_variety(Space space, Variety variety, RSpace rspace, RVariety rvariety, Thread thread);
AIndividuals next_individuals(AIndividuals aindividuals,
                              Space space, Variety variety, Individual individual,
                              RSpace rspace, RVariety rvariety,
                              RIndividualIn rindividual_in, RIndividualOut rindividual_out,
                              Thread thread);
...
struct _SSRIndividualsIn {
  UInt number;
  SRIndividualsIn srindividuals_in[];
};


struct _SSRIndividualsOut {
  UInt number;
  SRIndividualsOut srindividuals_out[];
};


struct _SRIndividualsIn {
  UInt64 number;
  RIndividualIn rindividual_in[];
};


struct _SRIndividualsOut {
  UInt64 number;
  RIndividualOut rindividual_out[];
...
struct _RWorld {
  ...
  SSRIndividualsIn ssrindividuals_in;
  SSRIndividualsOut ssrindividuals_out;
};
...
UInt64 z_xy(Float32 x,Float32 y, Float32 scale) {
  ...
  part[0] = z_table[(UInt8)(y_uint >>  0 & 0x0f) << 4 | (UInt8)(x_uint >>  0 & 0x0f)];
  part[1] = z_table[(UInt8)(y_uint >>  4 & 0x0f) << 4 | (UInt8)(x_uint >>  4 & 0x0f)];
  part[2] = z_table[(UInt8)(y_uint >>  8 & 0x0f) << 4 | (UInt8)(x_uint >>  8 & 0x0f)];
  part[3] = z_table[(UInt8)(y_uint >> 12 & 0x0f) << 4 | (UInt8)(x_uint >> 12 & 0x0f)];
  part[4] = z_table[(UInt8)(y_uint >> 16 & 0x0f) << 4 | (UInt8)(x_uint >> 16 & 0x0f)];
  part[5] = z_table[(UInt8)(y_uint >> 20 & 0x0f) << 4 | (UInt8)(x_uint >> 20 & 0x0f)];
  part[6] = z_table[(UInt8)(y_uint >> 24 & 0x0f) << 4 | (UInt8)(x_uint >> 24 & 0x0f)];
  part[7] = z_table[(UInt8)(y_uint >> 28 & 0x0f) << 4 | (UInt8)(x_uint >> 28 & 0x0f)];
...
//
IZ iz(Bool valid, UInt64 z) {
  IZ iz = { .valid = valid, .z = z };

  return iz;
}


// iz_valid_set: (IZ, Bool) -> (IZ)
//
// Duplicate of original IZ with valid set to valid (it is not possible to set it from invalid to valid).
//
IZ iz_valid_set(IZ iz, Bool valid) {
  // Invalid iz, done
  if (!iz.valid)
...
// iz_next_box: (IZ, UInt64,UInt64) -> (IZ)
//
...
// The functional pseudo-code follows.
//
// iz_next_box: (UInt64, UInt64,UInt64) -> (UInt64)
//
// iz_next_box(z, box_ul_z,box_lr_z)
//   // Interval [z+1,box_lr_z+1) contains Z value, find it
//   z < box_lr_z:
//     iz_next_box_out_size(z, 1, box_ul_z,box_lr_z)
//   // Interval [z+1,box_lr_z+1) does not contain Z value, done
//   otherwise:
//     fail("no more valid z points")
//
// iz_next_box_out_size(z, step, box_ul_z,box_lr_z)
//   // On step size boundary, increase step size
//   (z+1) % (step*4) == 0:
//     iz_next_box_out_size(z, step*4, box_ul_z,box_lr_z)
//   // Not on step size boundary, check box to next next boundary
//   otherwise:
//     iz_next_box_out_step(z, step, box_ul_z,box_lr_z)
//
// iz_next_box_out_step(z, step, box_ul_z,box_lr_z)
//   let ul_z = z+1
//   let lr_z = z+step
...
//     iz_next_box_in_size(z, step, box_ul_z,box_lr_z)
//   // Next box doesn't overlap with target box, skip over it
//   otherwise:
//     iz_next_box_out_size(lr_z, step, box_ul_z,box_lr_z)
//
// iz_next_box_in_size(z, step, box_ul_z,box_lr_z)
//   // Single point, done
//   step == 1:
...
//   otherwise:
//     iz_next_box_in_step(z, step/4, box_ul_z,box_lr_z)
//
// iz_next_box_in_step(z, step, box_ul_z,box_lr_z)
//   let ul_z = z+1
//       lr_z = z+step
...
//   otherwise:
//     iz_next_box_in_step(lr_z, step, box_ul_z,box_lr_z)
//
...
// iindividuals_valid_set: (IIndividuals, Bool) -> (IIndividuals)
//
// Duplicate of original IIndividuals with valid set to valid.
//
IIndividuals iindividuals_valid_set(IIndividuals iindividuals, Bool valid) {
  ...
  iindividuals_new.valid = valid;
  return iindividuals_new;
}


// iindividuals_index_set: (IIndividuals, UInt64, SIndividuals1) -> (IIndividuals)
//
...
IIndividuals iindividuals_next(IIndividuals iindividuals) {
  ...
  if (!iindividuals.valid || iindividuals.index+1 == iindividuals.number)
    return iindividuals_valid_set(iindividuals, false);

  // On different SIndividuals1, advance index and sindividuals1
  if ((iindividuals.index+1)%CLUSTER == 0)
    return iindividuals_index_set(iindividuals, iindividuals.index+1,
                                  sindividuals__sindividuals1(iindividuals.sindividuals_, iindividuals.index+1));
  // On same SIndividuals1, just advance index
  else
    return iindividuals_index_set(iindividuals, iindividuals.index+1, iindividuals.sindividuals1);
}


// iindividuals_first_z: (SIndividuals, IZ) -> (IIndividuals)
//
...
//
// iindividuals_first_z: (SIndividuals, UInt64) -> (UInt64)
// iindividuals_first_z_size: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
// iindividuals_first_z_step: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
//
// iindividuals_first_z(sindividuals, z)
...
//
// iindividuals_first_z_size(sindividuals, index, step, z):
...
//
// iindividuals_first_z_step(sindividuals, index, step, z)
...
IIndividuals iindividuals_first_z(SIndividuals sindividuals, IZ iz) {
  ...
  UInt64* right_z = sindividuals_.sindividuals0->right_z;
  UInt64 index = 0;
  UInt64 bit = 63;
...
// iindividuals_next_z: (IIndividuals, UInt64) -> (IIndividuals)
//
...
//
// iindividuals_next_z: (SIndividuals, UInt64, UInt64) -> (UInt64)
//
// iindividuals_next_z_out_size: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
// iindividuals_next_z_out_step: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
//
// iindividuals_next_z_in_size: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
// iindividuals_next_z_in_step: (SIndividuals, UInt64, UInt64, UInt64) -> (UInt64)
//
// iindividuals_next_z(sindividuals, index, z)
//   // Interval [index+1,number) contains individual, find it
//   sindividuals.number > 0 && sindividuals.z[sindividuals.number-1] >= z:
//     iindividuals_next_z_out_size(sindividuals, index, 1, z)
//   // Interval [index+1,number) does not contain individual, done
//   otherwise:
//     fail("no more valid individuals")
//
// iindividuals_next_z_out_size(sindividuals, index, step, z)
//   // On step size boundary, increase step size
//   (index+1) % (step*2) == 0:
//     iindividuals_next_z_out_size(sindividuals, index, step*2, z)
//   // Not on step size boundary, check interval to next boundary
//   otherwise:
//     iindividuals_next_z_out_step(sindividuals, index, step, z)
//
// iindividuals_next_z_out_step(sindividuals, index, step, z)
...
//     iindividuals_next_z_out_size(sindividuals, index+step, step, z)
//   // Interval [index+1,index+step+1) contains individual, locate individual in it
//   otherwise:
//     iindividuals_next_z_in_size(sindividuals, index, step, z)
//
// iindividuals_next_z_in_size(sindividuals, index, step, z)
//   // Single point, done
//   step == 1:
...
//     iindividuals_next_z_in_step(sindividuals, index, step/2, z)
//
// iindividuals_next_z_in_step(sindividuals, index, step, z)
//   // Interval [index+1,index+step+1) does not contain individual, skip over it
//   index+step+1 < sindividuals.number && sindividuals.z[index+step] < z
//     iindividuals_next_z_in_size(sindividuals, index+step, step, z)
//   // Interval [index+1,index+step+1) contains individual, locate individual in it
//   otherwise:
//     iindividuals_next_z_in_size(sindividuals, index, step, z)
//
...
IIndividuals iindividuals_next_z(IIndividuals iindividuals, IZ iz) {
  ...
  UInt64* right_z = iindividuals.sindividuals1->z;
  ...
  return iindividuals_index_set(iindividuals, index+1, sindividuals_.sindividuals1);
}


// iindividuals_first_box: (SIndividuals, UInt64,UInt64) -> (IIndividuals)
//
...
//
// iindividuals_first_box(sindividuals, box_ul_z,box_lr_z)
//   // Try one at or after box_ul_z first
//   let index = iindividuals_first_z(sindividuals, box_ul_z)
//   iindividuals_first_box_possible(sindividuals, index, box_ul_z,box_lr_z)
//
// iindividuals_first_box_possible(sindividuals, index, box_ul_z,box_lr_z)
...
//   otherwise:
//     let z_next = iz_next_box(sindividuals.z[index], box_ul_z,box_lr_z)
//         index_next = iindividuals_next_z(sindividuals, index, z_possible)
//     iindividuals_first_box_possible(sindividuals, index_next, box_ul_z,box_lr_z)
//
// Flattening the recursive calls with loops this becomes the following code.
//
IIndividuals iindividuals_first_box(SIndividuals sindividuals, UInt64 box_ul_z,UInt64 box_lr_z) {
  // Start with first individual after upper-left hand side of box
  IIndividuals iindividuals_new = iindividuals_first_z(sindividuals, iz(true, box_ul_z));
  ...
  while (1) {
    ...
    // Try next individual
    IZ iz_new = iz_next_box(iz(true, z), box_ul_z,box_lr_z);
    iindividuals_new = iindividuals_next_z(iindividuals_new, iz_new);
  }
...
// iindividuals_next_box: (IIndividuals, UInt64,UInt64) -> (IIndividuals)
//
...
// The functional pseudo-code follows.  The complexity of SIndividuals indexing is hidden in the [] operator.
//
// iindividuals_next_box(sindividuals, index, box_ul_z,box_lr_z)
//   // Try next individual first first
//   index+1 < sindividuals.number:
//     iindividuals_next_box_possible(sindividuals, index+1, box_ul_z,box_lr)
//   otherwise:
//     fail("no more valid individuals")
//
// iindividuals_next_box_possible(sindividuals, index, box_ul_z,box_lr_z)
//   // Individual is in box, done
//   z_overlap(individuals.z[index],individuals.z[index], box_ul_z,box_lr_z):
//     index
//   // Individual is not in box, try one at or after next Z in box next
...
//     let z_next = iz_next_box(sindividuals.z[index], box_ul_z,box_lr_z)
//         index_next = iindividuals_next_z(sindividuals, index, z_possible)
//     iindividuals_next_box_possible(sindividuals, index_next, box_ul_z,box_lr_z)
//
// Flattening the recursive calls with loops this becomes the following code.
//
IIndividuals iindividuals_next_box(IIndividuals iindividuals, UInt64 box_ul_z,UInt64 box_lr_z) {
...
AVarieties avarieties_begin() {
  ...
  if ( !(avarieties = malloc(sizeof(struct _SVarieties) + sizeof(Variety))) )
    die_errno(1, "unable to allocate %tu bytes for AVarieties",
...
AVarieties avarieties_append(AVarieties avarieties, Variety variety) {
  ...
  if ( avarieties->number > 0 && (avarieties->number & (avarieties->number-1)) == 0 ) {
    if ( !(avarieties_new = realloc(avarieties,
      ...
      die_errno(1, "unable to grow Avarieties allocation to %tu bytes",
                sizeof(struct _SVarieties) + sizeof(Variety)*(avarieties->number*2));
  }
  else
...
SVarieties avarieties_end(AVarieties avarieties) {
  ...
  if ( !(avarieties_new = realloc(avarieties,
    ...
    die_errno(1, "unable to shrink AVarieties allocation to %tu bytes",
              sizeof(struct _SVarieties) + sizeof(Variety)*avarieties->number);
...
ASIndividuals asindividuals_begin() {
  ...
  if ( !(asindividuals = malloc(sizeof(struct _SSIndividuals) + sizeof(SIndividuals))) )
    die_errno(1, "unable to allocate %tu bytes for ASIndividuals",
...
ASIndividuals asindividuals_append(ASIndividuals asindividuals, SIndividuals sindividuals) {
  ...
  if ( asindividuals->number > 0 && (asindividuals->number & (asindividuals->number-1)) == 0 ) {
    if ( !(asindividuals_new = realloc(asindividuals,
      ...
      die_errno(1, "unable to grow ASindividuals allocation to %tu bytes",
...
SSIndividuals asindividuals_end(ASIndividuals asindividuals) {
  ...
  if ( !(asindividuals_new = realloc(asindividuals,
    ...
    die_errno(1, "unable to shrink ASIndividuals allocation to %tu bytes",
...
SIndividuals aindividuals_end(AIndividuals aindividuals) {
  ...
  if (aindividuals.number > 0) {
    ...
    return sindividuals(aindividuals.number, sindividuals_);
  }
  // No Individuals, return empty SIndividuals
  else
...
//
// aindividuals_sort_both:  (*SIndividuals_, Int64,Int64, Int64) -> (SIndividuals_)
//
// In-place pivot sort.
//
...
SIndividuals_ aindividuals_sort(SIndividuals_ sindividuals_, UInt64 number, UInt64 pivot_z) {
  ...
  if (number > 0)
    return aindividuals_sort_both(sindividuals_,
...
SIndividuals_ aindividuals_sort_both(SIndividuals_ sindividuals_,
  ...
  if (left_z_min  < left_z_max)
    aindividuals_sort_both(sindividuals_,
...
SRVarieties srvarieties_begin(Space space, SVarieties svarieties) {
  ...
  if ( !(srvarieties = malloc(sizeof(struct _SRVarieties) +
    ...
    die_errno(1, "unable to allocate %tu bytes for SRVarieties",
              sizeof(struct _SRVarieties)+sizeof(struct _RVariety)*svarieties->number);

  // Initialize each RVariety with reduce_variety_first
  for (UInt index=0; index<svarieties->number; index+=1) {
    // Break out Variety
    Variety variety = svarieties->variety[index];

    // Initialize RVariety
    RVariety rvariety = reduce_variety_first(space, variety);
...
//---------------------------------------------------------------------------------------------------------------//
// srindividuals_in_begin: (Space, Variety, SIndividuals) -> (SRIndividualsIn)
//
// SRIndividualsIn prepared for reduction of sindividuals (initialized with reduce_individual_in_first).
//
SRIndividualsIn srindividuals_in_begin(Space space, Variety variety, SIndividuals sindividuals) {
  SRIndividualsIn srindividuals_in;

  // Create a RIndividualIn for each Individual in sindividuals
  if ( !(srindividuals_in = malloc(sizeof(struct _SRIndividualsIn)+
                                   sizeof(RIndividualIn)*sindividuals.number) ) )
    die_errno(1, "unable to allocate %tu bytes for SRIndividualsIn",
              sizeof(struct _SRIndividualsIn)+sizeof(RIndividualIn)*sindividuals.number);

  // Initialize each RIndividualIn with reduce_individual_in_first
  for ( IIndividuals iindividuals = iindividuals_first(sindividuals);
    ...
    RIndividualIn rindividual_in = reduce_individual_in_first(space, variety, individual);

    srindividuals_in->rindividual_in[iindividuals.index] = rindividual_in;
  }

  // Number of RIndividualsIn
  srindividuals_in->number = sindividuals.number;

  return srindividuals_in;
}


// srindividuals_in_end: (SRIndividualsIn) -> ()
//
// Release resources associated with srindividuals_in.
//
void srindividuals_in_end(SRIndividualsIn srindividuals_in) {
  free(srindividuals_in);
}


// srindividuals_out_begin: (Space, Variety, SIndividuals) -> (SRIndividualsOut)
//
// SEIndividualsOut prepared for reduction of sindividuals (initialized with reduce_individual_out_first).
//
SRIndividualsOut srindividuals_out_begin(Space space, Variety variety, SIndividuals sindividuals) {
  SRIndividualsOut srindividuals_out;

  // Create a RIndividualsOut for each Individual
  if ( !(srindividuals_out = malloc(sizeof(struct _SRIndividualsOut)+
                                    sizeof(RIndividualOut)*sindividuals.number) ) )
    die_errno(1, "unable to allocate %tu bytes for SRIndividualsOut",
              sizeof(struct _SRIndividualsOut)+sizeof(RIndividualOut)*sindividuals.number);

  // Initialize each RIndividualsOut with reduce_individual_out_first
  for ( IIndividuals iindividuals = iindividuals_first(sindividuals);
        iindividuals.valid;
        iindividuals = iindividuals_next(iindividuals) ) {
    // Break out Individual
    ...
    RIndividualOut rindividual_out = reduce_individual_out_first(space, variety, individual);

    srindividuals_out->rindividual_out[iindividuals.index] = rindividual_out;
  }

  // Number of RIndividualsOut
  srindividuals_out->number = sindividuals.number;

  return srindividuals_out;
}


// srindividuals_out_end: (SRIndividualsOut) -> ()
//
// Release resources associated with srindividuals_out.
//
void srindividuals_out_end(SRIndividualsOut srindividuals_out) {
  free(srindividuals_out);
}


//---------------------------------------------------------------------------------------------------------------//
// ssrindividuals_in_begin: (Space, SVarieties) -> (SSRIndividualsIn)
//
// SSRIndividualsIn prepared for reduction of ssindividuals (initialized with reduce_individual_in_first).
//
SSRIndividualsIn ssrindividuals_in_begin(Space space, SVarieties svarieties, SSIndividuals ssindividuals) {
  ...
  if ( !(ssrindividuals_in = malloc(sizeof(struct _SSRIndividualsIn) +
                                    sizeof(SRIndividualsIn)*number) ) )
    die_errno(1, "unable to allocate %tu bytes for SSRIndividualsIn",
              sizeof(struct _SSRIndividualsIn)+sizeof(SRIndividualsIn)*number);

  // Initialize each SRIndividualsIn with srindividuals_in_begin (uses reduce_individual_in_first)
  for (UInt index=0; index<number; index+=1) {
    ...
    SRIndividualsIn srindividuals_in = srindividuals_in_begin(space, variety, sindividuals);

    ssrindividuals_in->srindividuals_in[index] = srindividuals_in;
  }

  // Number of SRIndividualsIn
  ssrindividuals_in->number = number;

  return ssrindividuals_in;
}


// ssrindividuals_in_end: (SSRIndividualsIn) -> ()
//
// Release resources associated with ssrindividuals_in.
//
void ssrindividuals_in_end(SSRIndividualsIn ssrindividuals_in) {
  // Release SRIndividualsIn resources
  for (UInt index=0; index<ssrindividuals_in->number; index+=1) {
    // Break in SRIndividualsIn
    SRIndividualsIn srindividuals_in = ssrindividuals_in->srindividuals_in[index];

    // Release SRIndividualsIn resources
    srindividuals_in_end(srindividuals_in);
  }

  // Release SSRIndividualsIn
  free(ssrindividuals_in);
}


// ssrindividuals_out_begin: (Space, SVarieties) -> (SSRIndividualsOut)
//
// SSRIndividualsOut prepared for reduction of ssindividuals (initialized with reduce_individual_out_first).
//
SSRIndividualsOut ssrindividuals_out_begin(Space space, SVarieties svarieties, SSIndividuals ssindividuals) {
  UInt number = svarieties->number <= ssindividuals->number ? svarieties->number : ssindividuals->number;
  SSRIndividualsOut ssrindividuals_out;
  ...
  if ( !(ssrindividuals_out = malloc(sizeof(struct _SSRIndividualsOut) +
                                     sizeof(SRIndividualsOut)*number) ) )
    die_errno(1, "unable to allocate %tu bytes for SSRIndividualsOut",
              sizeof(struct _SSRIndividualsOut)+sizeof(SRIndividualsOut)*number);

  // Initialize each SRIndividualsOut with srindividuals_out_begin (uses reduce_individual_out_first)
  for (UInt index=0; index<number; index+=1) {
    // Break out (Variety,SIndividuals) pair
    Variety variety = svarieties->variety[index];
    SIndividuals sindividuals = ssindividuals->sindividuals[index];
    ...
    SRIndividualsOut srindividuals_out = srindividuals_out_begin(space, variety, sindividuals);

    ssrindividuals_out->srindividuals_out[index] = srindividuals_out;
  }

  // Number of SRIndividualsOut
  ssrindividuals_out->number = number;

  return ssrindividuals_out;
}


// ssrindividuals_out_end: (SSRIndividualsOut) -> ()
//
// Release resources associated with ssrindividuals_out.
//
void ssrindividuals_out_end(SSRIndividualsOut ssrindividuals_out) {
  // Release SRIndividualsOut resources
  for (UInt index=0; index<ssrindividuals_out->number; index+=1) {
    // Break in SRIndividualsOut
    SRIndividualsOut srindividuals_out = ssrindividuals_out->srindividuals_out[index];

    // Release SRIndividualsOut resources
    srindividuals_out_end(srindividuals_out);
  }

  // Release SSRIndividualsOut
  free(ssrindividuals_out);
}
...
void world_save_fp(FILE* file, World world, char* name) {
  ...
  // Save varieties
  UInt varieties_number = svarieties->number <= ssindividuals->number ? svarieties->number : ssindividuals->number;
...
// reduce_individual_*_bound functions give a bound which is used as a rough filter.  This set is then refined
// via the reduce_individual_*_filter functions.
//
...
//       rspace = reduce_space_first(space)
//
...
//       world_reduce_variety1((ssrindividuals_in, ssrindividuals_out, v0, i0), v1)
//         let // Inner Individual (index i1) reduction fold
//             ssrindividuals_in =
//               fold(world_reduce_individual1_in,
//                    (ssrindividuals_in, v0, i0, v1),
//                    indicies_filter_box(ssindividuals[v1],
//                                        reduce_individual_in_bound(space,
//                                                                   svarieties[v0], ssindividuals[v0][i0],
//                                                                   svarieties[v1])))
//             // Inner Individual (index i1) reduction fold
//             ssrindividuals_out =
//               fold(world_reduce_individual1_out,
//                    (ssrindividuals_out, v0, i0, v1),
//                    indicies_filter_box(ssindividuals[v1],
//                                        reduce_individual_out_bound(space,
//                                                                    svarieties[v0], ssindividuals[v0][i0],
//                                                                    svarieties[v1])))
//         (ssrindividuals_in, ssrindividuals_out, v0, i0)
//
//       // Outer Individual (index i0) reduction code
//       world_reduce_individual0((rspace, srvarieties, ssrindividuals_in, ssrindividuals_out, v0), i0)
//         let // Outer level RSpace and SRVariety reductions
//             rspace = reduce_space_merge(rspace,
//                                         reduce_space_rest(space, svarieties[v0], ssindividuals[v0][i0]))
//             srvarieties[v0] =
//               reduce_variety_merge(srvarieties[v0],
//                                    reduce_variety_rest(space, svarieties[v0], ssindividuals[v0][i0]))
//             // Outer Individual (index i0) reduction fold
//             (ssrindividuals_in, ssrindividuals_out) =
//               fold(world_reduce_individual0,
//                    (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out, v0, i0),
//                    indicies_all(ssindividuals[v0]))
//         (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out, v0)
//
//       // Outer Variety (index v0) reduction code
//       world_reduce_variety0((rspace, srvarieties, ssrindividuals_in, ssrindividuals_out), v0)
//         let // Outer Individual (index i0) reduction fold
//             (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out) =
//               fold(world_reduce_individual0,
//                    (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out, v0),
//                    indicies_all(ssindividuals[v0]))
//         (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out)
...
//       (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out) =
//         fold(world_reduce_variety0, (rspace, srvarieties, ssrindividuals_in, ssrindividuals_out),
...
RWorld world_reduce(World world) {
  ...
  // Reduction setup
  RSpace rspace = reduce_space_first(space);
  SRVarieties srvarieties = srvarieties_begin(space, svarieties);
  SSRIndividualsIn ssrindividuals_in = ssrindividuals_in_begin(space, svarieties, ssindividuals);
  SSRIndividualsOut ssrindividuals_out = ssrindividuals_out_begin(space, svarieties, ssindividuals);

  // Reduction outer loop over varieties
  UInt varieties_number =
    svarieties->number <= ssindividuals->number ? svarieties->number : ssindividuals->number;
  ...
  for (UInt varieties0_index=0; varieties0_index<varieties_number; varieties0_index+=1) {
    ...
    SRIndividualsIn srindividuals_in = ssrindividuals_in->srindividuals_in[varieties0_index];
    ...
    for ( IIndividuals iindividuals0 = iindividuals_first(sindividuals0);
      ...
      RIndividualIn rindividual_in = srindividuals_in->rindividual_in[iindividuals0.index];

      // Outer level reductions
      rspace = reduce_space_merge(rspace, reduce_space_rest(space,variety0,individual0));
      rvariety = reduce_variety_merge(rvariety, reduce_variety_rest(space,variety0,individual0));
      ...
      for (UInt varieties1_index=0; varieties1_index<varieties_number; varieties1_index+=1) {
        ...
        SRIndividualsOut srindividuals_out = ssrindividuals_out->srindividuals_out[varieties1_index];

        // Reduction inner loop over individuals in in loop
        Box box_in = reduce_individual_in_bound(space, variety0,individual0, variety1);
        UInt64 box_in_ul_z = z_xy(box_in.ul_x, box_in.ul_y, space.scale);
        UInt64 box_in_lr_z = z_xy(box_in.lr_x, box_in.lr_y, space.scale);

        for ( IIndividuals iindividuals1 = iindividuals_first_box(sindividuals1, box_in_ul_z,box_in_lr_z);
              iindividuals1.valid;
              iindividuals1 = iindividuals_next_box(iindividuals1, box_in_ul_z,box_in_lr_z) ) {
          ...
          if ( (varieties0_index != varieties1_index || iindividuals0.index != iindividuals1.index) &&
               reduce_individual_in_filter(space, variety0,individual0, variety1,individual1) )
            rindividual_in = reduce_individual_in_merge(rindividual_in,
                                                        reduce_individual_in_rest(space, variety0,individual0,
                                                                                  variety1,individual1));
        }

        // Reduction inner loop over individuals in out loop
        Box box_out = reduce_individual_out_bound(space, variety0,individual0, variety1);
        ...
        for ( IIndividuals iindividuals1 = iindividuals_first_box(sindividuals1, box_out_ul_z,box_out_lr_z);
              iindividuals1.valid;
              iindividuals1 = iindividuals_next_box(iindividuals1, box_out_ul_z,box_out_lr_z) ) {
          // Break out inner individual level components
          Individual individual1 = iindividuals_individual(iindividuals1);
          RIndividualOut rindividual_out = srindividuals_out->rindividual_out[iindividuals1.index];

          // Inner level reductions (individual to self is special case done in reduction setup)
          if ( (varieties0_index != varieties1_index || iindividuals0.index != iindividuals1.index) &&
               reduce_individual_out_filter(space, variety0,individual0, variety1,individual1) )
            rindividual_out = reduce_individual_out_merge(rindividual_out,
                                                          reduce_individual_out_rest(space, variety0,individual0,
                                                                                     variety1,individual1));

          // Write back reduction
          srindividuals_out->rindividual_out[iindividuals1.index] = rindividual_out;
        }
      }
      // Write back reduction
      srindividuals_in->rindividual_in[iindividuals0.index] = rindividual_in;
    }
  }

  // Package up and return the reduction
  RWorld rworld = { .rspace = rspace, .srvarieties = srvarieties,
                    .ssrindividuals_in = ssrindividuals_in, .ssrindividuals_out = ssrindividuals_out };
...
// world_next(space, svarieties, ssindividuals, rspace, srvarieties, ssrindividuals)
//   let // Next space
//       space_new = next_space(space, rspace);
//
//       // Individual (value individual, rindividual) construction code
//       world_next_individuals((aindividuals, variety, rvariety), (individual,rindividual_in, rindividual_out))
//         let // Next individuals
//             aindividuals =
//               next_individuals(aindividuals, world, variety, individual,
//                                rworld, rvariety, rindividual_in, rindividual_ou)
//         (aindividuals, variety, rvariety)
//
//       // Variety (value variety, rvariety) construction code
//       world_next_variety((), (variety, sindividuals, rvariety, srindividuals_in, srindividuals_out))
//         let // Next variety
//             variety_new = next_variety(space, variety, rspace, rvariety)
//             // Individual (value individual, rindividual) construction fold
//             aindividuals = aindividuals_begin()
//             aindividuals = fold(world_next_individuals,
//                                 (aindividuals, variety, rvariety),
//                                 zip(sindividuals, srindividuals_in, srindividuals_out))
...
World world_next(World world, RWorld rworld, Thread thread) {
  ...
  SSRIndividualsIn ssrindividuals_in = rworld.ssrindividuals_in;
  SSRIndividualsOut ssrindividuals_out = rworld.ssrindividuals_out;

  // Next space
  Space space_new = next_space(space, rspace, thread);
  ...
  varieties_number = varieties_number <= ssrindividuals_in->number ? varieties_number : ssrindividuals_in->number;
  varieties_number = varieties_number <= ssrindividuals_out->number ? varieties_number : ssrindividuals_out->number;
  ...
  for (UInt varieties_index=0; varieties_index<varieties_number; varieties_index+=1) {
    ...
    SRIndividualsIn srindividuals_in = ssrindividuals_in->srindividuals_in[varieties_index];
    SRIndividualsOut srindividuals_out = ssrindividuals_out->srindividuals_out[varieties_index];

    // Next variety
    Variety variety_new = next_variety(space, variety, rspace, rvariety, thread);
    ...
    individuals_number =
      sindividuals.number <= srindividuals_in->number ? sindividuals.number : srindividuals_in->number;
    individuals_number =
      individuals_number <= srindividuals_out->number ? individuals_number : srindividuals_out->number;
    ...
    for ( IIndividuals iindividuals = iindividuals_first(sindividuals);
      ...
      Individual individual = iindividuals_individual(iindividuals);
      RIndividualIn rindividual_in = srindividuals_in->rindividual_in[iindividuals.index];
      RIndividualOut rindividual_out = srindividuals_out->rindividual_out[iindividuals.index];

      // Next individual
      aindividuals = next_individuals(aindividuals, space, variety, individual,
                                      rspace, rvariety, rindividual_in, rindividual_out,
                                      thread);
    }
  ...
  // Package up and return components
  World world_new = { .space = space_new, .svarieties = svarieties_new, .ssindividuals = ssindividuals_new };
...
void rworld_end(RWorld rworld) {
  ...
  ssrindividuals_in_end(rworld.ssrindividuals_in);
  ssrindividuals_out_end(rworld.ssrindividuals_out);
}
...
Box reduce_individual_out_bound(Space space, Variety variety0, Individual individual1, Variety variety1) {
  Box box = { };
  return box;
}
